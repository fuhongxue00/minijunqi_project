...
## 最新进度
- **修改几处bug之后，已经成功验证训练有效：和随机初始化的AI进行对局30局，0败9和21胜，换颜色验证结果也类似**
- 具体做法（bug修复和训练操作）：将学习率调成1e-1后，会发现训练后行为明显有变化，但出现bug，问题在于masksoftmax给出的负值不够大，把负值改为负无穷之后，稍加调参后发现单次训练强化学习在三十局内3胜27和，于是用这个checkpoint开始再训30轮，成功达到当前结果。
- 具体做法（奖励的设置）之前的和棋奖励设置有问题，目前的设置：胜方全局+10 负全局-5 和也全局-5。主动吃子单步+20 * index/len(traj)，主动送子-1
- 目前最优的chekpoint在checkpoints/best.pt
