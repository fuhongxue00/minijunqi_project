...
## 最强checkpoint情况
- checkpoints/best0905_1 使用随机初始化，训练30轮，学习率初始化为1e-2，训练过程中学习率每2轮衰减为0.9，奖励包含吃子、送子以及胜负关系奖励
- checkpoints/best0905_2 使用checkpoints/best0905_1的参数初始化，训练配置同上
- checkpoints/best0906_3 使用checkpoints/best0905_2的参数初始化，学习率初始化为1e-3，每2轮衰减为0.9，奖励只包含胜负关系 胜10负-10 和0
- 上述checkpoint逐一强于前一版本,已经过验证
...
- 持续检查潜在bug，减小屎山
- 更新调参，看能不能超过人类
- 增加几个类似RNN的通道（已预留好），可以认为这几个通道代表对对方信息的猜测
- 如需扩大规模，则需要先把张量的batch搞好
- 看deepnash论文，改强化学习策略。
- 后续：如果棋子移动已经明显能看出训练得足够好了，则将规则复杂化（大本营不能移动，从而可以看出部署的训练有无效果）这个只是简单实现即可
- 再后续：考虑实现四国棋盘下的一对一军棋，或直接实现双明军棋，这个前期尽量避免屎山，则尝试起来也比较快
